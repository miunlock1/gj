...
function change(){
        ...
        if(strpos($claim, 'Promo kamu sudah bisa dipakai pak')){
        echo "\n".color("green","+] Message: ".$message);
        goto gofood;
        }else{
        echo "\n".color("red","-] Message: ".$message);
        ...
        gofood:
        echo "\n".color("yellow","!] Claim voc 20k 5 voucher");
        echo "\n".color("yellow","!] tunggu um");
        for($a=1;$a<=3;$a++){
        echo color("yellow",".");
        sleep(3);
        }
        $code1 = request('/go-promotions/v1/promotions/enrollments', $token, '{"promo_code":"G-CSPCZKC"}');
        $message = fetch_value($code1,'"message":"','"');
        if(strpos($code1, 'Promo kamu sudah bisa dipakai pak')){
        echo "\n".color("green","+] Message: ".$message);
        sleep(1);
        ...
        $expired6 = getStr1('"expiry_date":"','"',$cekvoucher,'6');
         ...
         echo color("red","========( PIN ANDA = 212121 )========")."\n";
         $data2 = '{"pin":"212121"}';
